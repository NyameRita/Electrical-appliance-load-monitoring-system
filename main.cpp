...
void showEnergySummary(const vector<Appliance>& appliances) {
    ...
    double total = 0.0;
    for (const auto& a : appliances) total += a.energyKWhPerDay();
...
void calculateBill(const vector<Appliance>& appliances) {
    ...
    for (const auto& a : appliances) totalEnergy += a.energyKWhPerDay();
...
int main() {
    ...
    while (true) {
        ...
        switch (choice) {
            case 1: appliances.push_back(registerAppliance()); break;
            case 2: viewAllAppliances(appliances); break;
            case 3: searchApplianceByName(appliances); break;
            case 4: showEnergySummary(appliances); break;
            case 5: calculateBill(appliances); break;
            case 0: cout << "Goodbye!\n"; return 0;
